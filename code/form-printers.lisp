...
(declaim (inline newline-type newline-type-ext))

(defun newline-type (fillp)
  (if fillp :fill :linear))

(defun newline-type-ext (colon-p at-sign-p)
  (if colon-p
      (if at-sign-p :mandatory :fill)
      (if at-sign-p :miser :linear)))
...
(defmethod pprint-defpackage (client stream object &optional colon-p at-sign-p)
  (let ((primary-newline (newline-type colon-p)))
    (pprint-logical-block (client stream object :prefix "(" :suffix ")")
      ...
      (loop (pprint-exit-if-list-exhausted)
            (write-char #\Space stream)
            (pprint-newline client stream primary-newline)
...
(defmethod pprint-defun (client stream object &optional colon-p at-sign-p)
  (let ((primary-newline (newline-type colon-p))
        (secondary-newline (newline-type at-sign-p)))
    (pprint-body-form (client stream object :paren t :newline secondary-newline)
      (pprint-exit-if-list-exhausted)
      (incless:write-object client (pprint-pop) stream)
      (pprint-exit-if-list-exhausted)
      (pprint-indent client stream :block 3)
...
(defmethod pprint-do (client stream object &optional colon-p at-sign-p)
  (pprint-tagbody-form (client stream object :newline (newline-type at-sign-p))
    (pprint-exit-if-list-exhausted)
    (incless:write-object client (pprint-pop) stream)
    (pprint-exit-if-list-exhausted)
    (pprint-indent client stream :block 3)
    ...
    (pprint-exit-if-list-exhausted)
    (write-char #\Space stream)
    (pprint-newline client stream (newline-type colon-p))
...
(defmethod pprint-let (client stream object &optional colon-p at-sign-p)
  (declare (ignore colon-p))
  (pprint-body-form (client stream object :paren t :newline (newline-type at-sign-p))
    (pprint-exit-if-list-exhausted)
    (incless:write-object client (pprint-pop) stream)
    (pprint-exit-if-list-exhausted)
    (write-char #\Space stream)
...
(defmethod pprint-argument-list (client stream object
                                 &optional colon-p at-sign-p argument-count)
  (declare (ignore at-sign-p))
  (pprint-function-call-form (client stream object :argument-count argument-count :newline (newline-type colon-p))))

(defmethod pprint-with (client stream object &optional colon-p at-sign-p argument-count)
  (pprint-body-form (client stream object :paren t :newline (newline-type colon-p))
    (pprint-exit-if-list-exhausted)
    (incless:write-object client (pprint-pop) stream)
    (pprint-exit-if-list-exhausted)
    (pprint-indent client stream :block 3)
    (write-char #\Space stream)
    (pprint-newline client stream :miser)
    (pprint-argument-list client stream (pprint-pop) at-sign-p nil argument-count)))

(defmethod pprint-lambda-list (client stream object &optional colon-p at-sign-p)
  (let ((argument-newline (newline-type colon-p))
        (group-newline (newline-type at-sign-p)))
    (pprint-format-logical-block (client stream object :paren t)
      (let ((state :required)
            (first t)
            (group nil))
                ...
                (unless first
                  (case arg
                    (&optional
                     ...
                     (pprint-newline client stream group-newline))
                    ((&rest &body)
                     (setf state :required
                           group t)
                     (pprint-indent client stream :block 0)
                    ...
                    (&aux
                     ...
                     (pprint-newline client stream group-newline))
                    (otherwise
                     (pprint-newline client stream argument-newline))))
                (cond
                  ((eq state :required)
...
(defmethod pprint-case (client stream object &optional colon-p at-sign-p)
  (declare (ignore colon-p))
  (let ((secondary-newline (newline-type at-sign-p)))
    (pprint-format-logical-block (client stream object :paren t)
      (pprint-exit-if-list-exhausted)
      (incless:write-object client (pprint-pop) stream)
      (pprint-exit-if-list-exhausted)
      ...
      (pprint-indent client stream :block 1)
      (write-char #\Space stream)
      (pprint-newline client stream secondary-newline)
